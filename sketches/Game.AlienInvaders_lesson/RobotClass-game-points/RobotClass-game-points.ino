...
void drawPlayerRocket(){
    if( is_rocket_flying )
        display.drawPixel(rocket_coords[X], rocket_coords[Y], 1);
}

void firePlayerRocket(){
    is_rocket_flying = 1;
    rocket_coords[X] = player_coords[X]+UNIT_WIDTH/2;
    rocket_coords[Y] = player_coords[Y]-1;
}

void rocketMove(){
    if( rocket_coords[Y] == 0 ){
        is_rocket_flying = 0;
    }
    rocket_coords[Y] -= 1;
}
...
void loop() {
    ...
    if( t > rocket_fire_next && !is_rocket_flying ){
